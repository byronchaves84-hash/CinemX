...
ACTIVITY_FILE = os.path.join(CINEMA_DIR, "activity.json")
...
class CinemaXHandler(
    ...
    def load_activity(self):
        with ACTIVITY_LOCK:
            return self.load_json_file(ACTIVITY_FILE)
    ...
    def add_activity(self):
        try:
            data = self.read_body()
            activity = {
                "id": clean_id(data.get("id")) or str(uuid.uuid4()),
            ...
            activities = self.load_activity()
            activities.insert(0, activity)
            activities = activities[:100]

            if not self.save_activity(activities):
                self.send_json({
                    "success": False,
    ...
    def delete_activity(self, activity_id):
        try:
            activity_id = clean_id(activity_id)
            ...
            remaining = [
                ...
                if clean_id(activity.get("id")) != activity_id
            ]
            ...
            if not self.save_activity(remaining):
                self.send_json({
                    "success": False,
    ...
    def clear_activity(self):
        try:
            if not self.save_activity([]):
                self.send_json({
                    "success": False,
                    "error": "No se pudo vaciar la actividad."
                }, 500)
                return

            self.send_json({
                "success": True,
                "message": "Actividad vaciada correctamente.",
                "activity": [],
                "data": [],
                "total": 0
            })
            ...
            print("❌ ERROR vaciando actividad:", e)
            self.send_json({
                "success": False,
                "error": "Error vaciando actividad.",
                "details": str(e)
            }, 500)
    ...
    def do_POST(
        ...
        if path.startswith("/api/users/session/") and path.endswith("/disconnect"):
            ...
            session_id = path[
            len("/api/users/session/"):-len("/disconnect")
            ]
    ...
    def do_DELETE(
        ...
        if path.startswith("/api/activity/"):
            activity_id = clean_id(path[len("/api/activity/"):])
